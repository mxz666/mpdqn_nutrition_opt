...
class MultiPassQActor(nn.Module):
    ...
    def forward(self, state, action_parameters):
        ...
        # with torch.no_grad():
        # print("action_parameters in MPDQN ", action_parameters, " shape ", action_parameters.shape, " batch_size ", batch_size)
        x = torch.cat((state.to(torch.float16), torch.zeros_like(action_parameters.to(torch.float16))), dim=1)
        ...
        Qall = self.layers[-1](x).to(torch.float16)
